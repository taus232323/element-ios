...
swift run --package-path Tools -q tools ci upload-dsyms --dsym-path "$CI_ARCHIVE_PATH/dSYMs"

generate_what_to_test_notes

if [ "$CI_WORKFLOW" = "Release" ]; then
    swift run --package-path Tools -q tools ci release-to-github
elif [ "$CI_WORKFLOW" = "Nightly" ]; then
    swift run --package-path Tools -q tools ci tag-nightly --build-number "$CI_BUILD_NUMBER"
fi
